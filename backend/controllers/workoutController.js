  ...
  async (req, res) => {
    try {
      ...
      */
       ...
       * -------------------------------------------------------------------------
       * Clears an unfinished workout log so a new workout can start from 00:00.
       * This is intentionally an explicit action; normal "start" still resumes
       * an existing in-progress workout.
       */
      if (req.body?.action === "reset") {
        const context = await resolveWorkoutTimerContext(req, res)
        if (!context) return

        let { workoutLog } = context

        if (workoutLog.completed) {
          return res.status(400).json({
            success: false,
            message: "This workout has already been completed and cannot be reset.",
            workoutLog,
          })
        }
        ...
        return res.status(200).json({
          success: true,
          message: "Workout reset. A new workout can now start from 00:00.",
          workoutLog,
        })
      }
...
/*
|--------------------------------------------------------------------------
| COMPATIBILITY — MEMBER WORKOUT SCHEDULE
|--------------------------------------------------------------------------
| These handlers keep the existing workoutRoutes.js endpoints working.
| They use the same ProgramAssignment data already used by this controller.
|--------------------------------------------------------------------------
*/

const getAssignmentForCalendarDate = async (memberId, date) => {
...
const getTodayWorkout = async (req, res) => {
  try {
    ...
    const assignment = await getAssignmentForCalendarDate(
      req.user._id,
      today,
    )
    ...
    return res.status(200).json({
      ...
      hasWorkout: Boolean(assignment),
      date: getStartOfDay(today),
      assignment: assignment || null,
      workout: assignment
        ? {
            assignmentId: assignment._id,
            workoutDate: assignment.workoutDate,
    ...
    return res.status(500).json({
      success: false,
      message: "Unable to retrieve today's workout.",
    })
  }
}
...
const getTomorrowWorkout = async (req, res) => {
  try {
    ...
    console.error("Get tomorrow workout error:", error)
    return res.status(500).json({
      success: false,
      message: "Unable to retrieve tomorrow's workout.",
    })
  }
}

const getWorkoutForDate = async (req, res) => {
  try {
    const requestedDate =
      req.query?.date ||
      req.params?.date ||
    ...
    return res.status(200).json({
      ...
      assigned: Boolean(assignment),
      hasWorkout: Boolean(assignment),
      date: getStartOfDay(workoutDate),
      assignment: assignment || null,
      workout: assignment
        ? {
            assignmentId: assignment._id,
    ...
    console.error("Get workout for date error:", error)
    return res.status(500).json({
      success: false,
      message: "Unable to retrieve the workout for this date.",
    })
  }
}
...
const getWeeklySchedule = async (req, res) => {
  try {
    ...
    console.error("Get weekly workout schedule error:", error)
    return res.status(500).json({
      success: false,
      message: "Unable to retrieve the weekly workout schedule.",
    })
  }
}
